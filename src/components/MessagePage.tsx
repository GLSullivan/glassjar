        ...
        {Object.keys(groupedMessages).length > 0 ? (
          <>
            {Object.keys(groupedMessages).map((accountId) => (
              <div
                key={accountId}
                className="glassjar__list-item glassjar__list-item--account glassjar__list-item--message"
              >
                <div
                  className="glassjar__list-item__header"
                  onClick={() => {
                    dispatch(
                      setActiveAccount(groupedMessages[accountId][0].account)
                    );
                ...
                <MessagesList
                  account={groupedMessages[accountId][0].account}
                  color={solidTransparentColor(accountColors[
                    groupedMessages[accountId][0].account.color
                  ],.1)}
                ></MessagesList>
                <div
